...
Partial Public Class Rcptv
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If IsPostBack = False Then
            ...
            cboxMemberName.Items.Insert(0, New ListItem("-- SELECT MEMBER --", -1))

            cboxSname.DataSource = dt
            cboxSname.DataTextField = "Member_Name"
            cboxSname.DataValueField = "Ejamaat"
            cboxSname.DataBind()
            cboxSname.Items.Insert(0, New ListItem("-- SELECT MEMBER --", -1))

            'For i As Integer = 0 To dt.Rows.Count - 1
            '    cboxSname.Items.Add(dt.Rows(i).ItemArray(0).ToString)
            'Next

            'da = New OleDb.OleDbDataAdapter("SELECT Member_Name FROM PartyLedger WHERE Group_Leader='" & lblLogged.Text & "' AND Status = 'Active' ORDER BY Member_Name", con)
            'dt = New DataTable
            'da.Fill(dt)
    ...
    Private Sub DatagridUpdate()
        da = New OleDb.OleDbDataAdapter("SELECT JVNo, DOJ, Account_Name, Narration, FCY, Amount, Sign FROM JournalEntry WHERE Sign='Cr' ORDER BY JVNo DESC", con)
        ds = New DataSet
        da.Fill(ds)
